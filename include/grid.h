...
public:
    ...
    void InsertXYZI_with_origin(PointType2 point_in, int cnt)
    {
        ...
        if(((point_in.y<(range_y_-center_y_))&&(point_in.y>center_y_))
                &&((point_in.x<(range_x_-center_x_))&&(point_in.x>(center_x_))))
        {

            n_pos = int((point_in.y - center_y_)/res_);
            m_pos = int((point_in.x - center_x_)/res_);
...
//                    double min_z = 0.8*abs(sqrt(dist_x*dist_x + dist_y*dist_y)) * tan(PI*2.0/180);
    ...
    {
        bool keep_flag;
	map_->resize(grid_number_m_*grid_number_n_);
	int size = 0;
        for(int cnt_m=0;cnt_m<grid_number_m_;cnt_m++)
        {
            for(int cnt_n=0;cnt_n<grid_number_n_;cnt_n++)
